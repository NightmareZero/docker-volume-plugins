...
build() {
    ...
    if [ $ARCH == "x86_64" ]
    then
        docker plugin rm -f nightzhuxy/$1:$TAG || true
    else
        docker plugin rm -f nightzhuxy/$1-$ARCH:$TAG || true
    fi
    docker rmi -f rootfsimage || true
    docker buildx build --load --platform ${BPLATFORM} \
        --build-arg GO_VERSION=1.15.10 \
        --build-arg UBUNTU_VERSION=22.04 \
        -t rootfsimage -f $1/Dockerfile --network host .
    ...
    if [ $ARCH == "x86_64" ]
    then
        docker plugin create nightzhuxy/$1:$TAG build
        docker plugin push nightzhuxy/$1:$TAG
    else
        docker plugin create nightzhuxy/$1-$ARCH:$TAG build
        docker plugin push nightzhuxy/$1-$ARCH:$TAG
    fi
}
